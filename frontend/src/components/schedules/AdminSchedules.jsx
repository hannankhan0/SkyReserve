...
const AdminSchedules = () => {
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      if (editId) {
        ...
        toast.success('Schedule created and seats generated.');
      }
      reset(); load();
    } catch (err) {
      toast.error(err.response?.data?.message || 'Schedule save failed. Use full valid date/time and allowed status.');
    }
  };
  ...
  const remove = async (id) => {
    ...
    try { await api.delete(`/schedules/${id}`); toast.success('Schedule deleted.'); load(); }
