...
const SeatSelection = () => {
  ...
  const passengerCount = Number(state?.passengerCount || 1);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      toast.error(backendMessage || 'Failed to load seat map. Please check backend and database.');
    } finally {
      setLoading(false);
    }
  }, [scheduleId, toast]);
  ...
  const selectedSeats = useMemo(() => seats.filter((s) => selected.includes(s.seat_id)), [seats, selected]);
  const total = useMemo(() => selectedSeats.reduce((sum, s) => sum + Number(s.seat_price || 0), 0), [selectedSeats]);
  ...
  const cabinSummary = useMemo(() => grouped.map(([klass, klassSeats]) => {
    const available = klassSeats.filter(isAvailable).length;
    const minPrice = klassSeats.reduce((min, seat) => Math.min(min, Number(seat.seat_price || Infinity)), Infinity);
    return { klass, total: klassSeats.length, available, minPrice: Number.isFinite(minPrice) ? minPrice : 0 };
  }), [grouped]);

  const toggleSeat = (seat) => {
    if (!isAvailable(seat)) return;
    setSelected((prev) => (prev.includes(seat.seat_id) ? prev.filter((id) => id !== seat.seat_id) : [...prev, seat.seat_id]));
  };
  ...
  const proceed = async () => {
    ...
    try {
      ...
      setTimeout(() => {
        navigate('/bookings/create', { state: { schedule, selectedSeats, totalAmount: total, holdExpiresAt: expiresAt } });
      }, 700);
    } catch (err) {
      toast.error(err.response?.data?.message || 'Seat reserve failed. Refresh and try again.');
      await load();
    } finally {
      setReserving(false);
    }
    ...
    return (
      <div className="page-container seat-page">
        <div className="seat-map-skeleton">
          ...
          <div className="skeleton-plane">{Array.from({ length: 36 }).map((_, index) => <span key={index}></span>)}</div>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="page-container seat-page">
      <div className="page-header">
        <div><h1>Select Seats</h1><p>{schedule?.flight_number || 'Flight'} - {schedule?.departure_city || 'From'} to {schedule?.destination_city || 'To'}</p></div>
      ...
      <div className="cabin-summary-bar">
        {cabinSummary.map((item) => <div className="cabin-summary-pill" key={item.klass}><strong>{classLabel(item.klass)}</strong><span>{item.available}/{item.total} seats</span><em>from {formatPKR(item.minPrice)}</em></div>)}
      </div>

      <div className="airplane-card">
        <div className="airplane-nose"><span></span><strong>Cockpit</strong></div>
        <div className="airplane-body">
          {grouped.length === 0 ? (
            <div className="empty-state">No seats generated for this schedule yet.</div>
          ) : (
            grouped.map(([klass, klassSeats], cabinIndex) => (
              <div key={klass} className="cabin-section">
                {cabinIndex > 0 && <div className="cabin-divider"><span></span></div>}
                ...
                <div className="airplane-seat-grid">
                  {klassSeats.map((seat, index) => {
                    ...
                    return (
                      <button type="button" key={seat.seat_id} className={`seat airplane-seat ${status} ${selectedClass}${aisleAfter}`} onClick={() => toggleSeat(seat)} disabled={!isAvailable(seat)} title={`${seat.seat_number} | ${classLabel(seat.seat_class)} | ${status} | ${formatPKR(seat.seat_price)}`}>
      ...
      <div className="card summary-card sticky-booking-summary">
        ...
        <p><strong>Seats picked:</strong> {selectedSeats.length}/{passengerCount} passenger{passengerCount === 1 ? '' : 's'}</p>
        ...
        <button className="btn-primary" disabled={reserving || selected.length !== passengerCount} onClick={proceed}>{reserving ? 'Reserving seats...' : 'Reserve Selected Seats & Proceed to Booking'}</button>
      </div>
    </div>
  );
};
