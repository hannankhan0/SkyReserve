// ─────────────────────────────────────────────
// INPUT VALIDATORS  (normalized schema)
// ─────────────────────────────────────────────
 ...
 * Normalized schema: departure_city_id / destination_city_id are INT FKs → Cities.
 * City names are no longer stored directly on Flights.
 */
const validateFlight = (body, isUpdate = false) => {
    ...
    if (!isUpdate) {
        ...
        if (!departure_city_id || isNaN(departure_city_id) || parseInt(departure_city_id) <= 0)
            errors.push('departure_city_id is required and must be a positive integer (from the Cities table).');
        if (!destination_city_id || isNaN(destination_city_id) || parseInt(destination_city_id) <= 0)
            errors.push('destination_city_id is required and must be a positive integer (from the Cities table).');
        if (departure_city_id && destination_city_id &&
            parseInt(departure_city_id) === parseInt(destination_city_id))
            errors.push('departure_city_id and destination_city_id must be different.');
        ...
        if (departure_city_id !== undefined &&
            (isNaN(departure_city_id) || parseInt(departure_city_id) <= 0))
            errors.push('departure_city_id must be a positive integer.');
        if (destination_city_id !== undefined &&
            (isNaN(destination_city_id) || parseInt(destination_city_id) <= 0))
            errors.push('destination_city_id must be a positive integer.');
        if (departure_city_id !== undefined && destination_city_id !== undefined &&
            parseInt(departure_city_id) === parseInt(destination_city_id))
            errors.push('departure_city_id and destination_city_id must be different.');
