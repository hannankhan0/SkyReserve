...
// Only trust user_id from the verified JWT payload — never from body or headers
const getUserIdFromRequest = (req) => {
  if (req.user?.role_type === 'admin') return null; // admin token, not a user token
...
const createBooking = async (req, res) => {
  try {
    ...
    if (!user_id) return res.status(403).json({ success: false, message: 'A passenger account is required. Please log out and log in as a user (not admin).' });
