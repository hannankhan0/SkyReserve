...
const getUserIdFromRequest = (req) => {
  if (req.user?.role_type === 'admin') return null;
...
const getTicketsByBookingReference = async (req, res) => {
  try {
    const reference = String(req.params.reference || '').trim().toUpperCase();
    if (!reference) return res.status(400).json({ success: false, message: 'Booking reference is required' });
    const tickets = await Ticket.getTicketsByBookingReference(reference);
    res.json({ success: true, count: tickets.length, data: tickets });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message });
  }
...
module.exports = { getMyTickets, getTicketById, getTicketsByBookingReference, checkInTicket, downloadTicket };
