...
const Booking = {
  async createBooking({ user_id, schedule_id, passengers, special_requests }) {
    ...
    try {
      ...
      // ── Fetch schedule + flight info (no available_seats column) ────────
      ...
      // ── Fetch seat details — price_multiplier from Seat_Class_Prices ────
      ...
      const seatResult = await seatRequest.query(`
        SELECT
          ...
          s.hold_user_id
        FROM Seats s
        JOIN Seat_Class_Prices scp ON s.seat_class = scp.seat_class
        WHERE s.schedule_id = @schedule_id
          AND s.seat_id IN (${seatPlaceholders})
      `);
      // ────────────────────────────────────────────────────────────────────
      ...
      insertBookingRequest.input('special_requests',  sql.VarChar(sql.MAX), special_requests || null);
      ...
      const seatUpdatePlaceholders = seatIds.map((_, index) => `@seatUpdate${index}`).join(', ');
      ...
      // NOTE: No UPDATE to Flight_Schedules.available_seats — that column was
      // removed in the normalized schema. Availability is computed live.
  ...
  async cancelBooking(booking_id) {
    ...
    try {
      ...
      // NOTE: No UPDATE to Flight_Schedules.available_seats — column removed.
      // The seat release above is enough; availability is computed live.
