  ...
  async createBooking({ user_id, schedule_id, passengers, special_requests }) {
    ...
    try {
      ...
      const seatResult = await seatRequest.query(`
        SELECT seat_id, seat_number, seat_class, is_available, price_multiplier
        FROM Seats
      ...
      const unavailableSeat = seatResult.recordset.find(
      ...
      for (let i = 0; i < passengers.length; i++) {
        ...
        const ticketNumber = `TKT-${schedule.flight_number}-${Date.now()}-${i + 1}`;
